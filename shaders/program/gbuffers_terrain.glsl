...
void main() {
	...
	if (albedocheck > 0.00001) {
		...
		float ao = 1.0;
			...
			if (materialFormat > 0.5) {
				...
				float netherStems    = float(eminMat > 10.98 && eminMat < 11.02);
				      cauldron       = float(eminMat > 11.98 && eminMat < 12.02);
				float ancientDebris  = float(eminMat > 12.98 && eminMat < 13.02);
				float fire           = float(eminMat > 13.98 && eminMat < 14.02);
				float redstone       = float(eminMat > 14.98 && eminMat < 15.02);
				float gem 			 = float(eminMat > 15.98 && eminMat < 16.02);
		...
		/*
		float texNoise1 = texture2D(noisetex, 0.125 * (worldPos.xz + cameraPosition.xz)).r;
		float texNoise2 = texture2D(noisetex, 0.125 * (worldPos.xy + cameraPosition.xy)).r;
		float texNoise3 = texture2D(noisetex, 0.125 * (worldPos.yz + cameraPosition.yz)).r;
		float texNoise = texNoise1 + texNoise2 + texNoise3;
		albedo.rgb *= 0.5 + texNoise * 0.25;
		*/
		...
		float materialAO = 1.0;
			...
			//if (metalness > 0.01) albedo.rgb = vec3(1.0, 0.0, 1.0) * 2.0;
					...
					parallaxShadow, emissive + definite_emissive, subsurface, mat, leaves, scattering, materialAO);
						...
						if (quarterNdotUp > 0.85) {
							...
							if (no_rain_ref < 0.1) {
								...
								pnormalMap += (texture2D(noisetex, rainPos + vec2(wind.x, wind.y)).xyz - vec3(0.5)) * pnormalMultiplier;
								pnormalMap += (texture2D(noisetex, rainPos + vec2(wind.x * -1.5, wind.y * -1.0)).xyz - vec3(0.5)) * pnormalMultiplier;
	...
	gl_FragData[3] = vec4(EncodeNormal(newNormal), materialFormatFlag, 1.0);
...
void main(){
	...
	if (mc_Entity.x ==  10) // Lava
		mat = 3.0, quarterNdotUfactor = 0.0, color.a *= 0.16, color.rgb *= sqrt(LAVA_BRIGHTNESS);
	if (mc_Entity.x ==  1010) // Fire
		mat = 3.0, lmCoord.x = 0.5, color.a *= 0.27 - compatibilityFactor * 0.09, color.rgb *= sqrt(FIRE_BRIGHTNESS), eminMat = 14.0;
	...
	if (mc_Entity.x == 12345) // Custom Emissive
		...
		if (mc_Entity.x == 55) // Redstone Wire
			eminMat = 15.0;
		if (mc_Entity.x == 7777) // Diamond Block, Emerald Block
			eminMat = 16.0;
