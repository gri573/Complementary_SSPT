	...
	float CloudCoverage(float noise, float coverage, float NdotU, float cosS) {
		...
		noiseCoverageCosS *= noiseCoverageCosS;
		//noiseCoverageCosS *= timeBrightnessS + (1.0 - sunVisibility);
	...
	vec4 DrawCloud(vec3 viewPos, float dither, vec3 lightCol, vec3 ambientCol, float NdotU, int sampleCount) {
		...
		if (NdotU > 0.0) {
			...
			vec3 cloudUpColor = mix(lightNight * lightNight * lightNight * 13.5 * (1.0 + 3.0 * nightVision), pow(lightCol, vec3(1.5)) * 1.75, sunVisibility4);
	...
	float CloudNoiseEnd(vec2 coord, vec2 wind) {
		...
		vec4 DrawEndCloud(vec3 viewPos, float dither, vec3 lightCol) { // Ender Nebula
			...
			NdotUM = 1.0 - NdotUM;
			//NdotUM *= NdotUM;
			//NdotUM = pow(NdotUM, NEBULA_DISTRIBUTION);
			//NdotUM = smoothstep(0.0, 1.0, NdotUM) * 0.75;
			//NdotUM *= 0.85;
			...
			//aurora *= pow(min(NdotU, 0.0) + 1.0, 3.0);
